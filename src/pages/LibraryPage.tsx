...
export default function LibraryPage() {
  ...
  const { isGenerating: isLevelingGenerating, generateLevelingGuide } = useLevelingGuide();
  ...
  const handleLevelingGuide = async (c: Character) => {
    setActionLoading({ id: c.id, type: 'level' });
    const content = await generateLevelingGuide(c);
    if (content) {
      // Save to PDF using play guide PDF hook format
      await generatePlayGuidePdf({
        characterName: c.character_name,
        characterClass: c.character_class,
        race: c.race,
        level: c.level,
        playGuideContent: content,
      });
      // Refresh so leveling_guide_content is cached locally
      refetch();
    }
    setActionLoading(null);
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 animate-fade-in">
          ...
          <Link to="/">
            <Button variant="gold" size="lg" className="gap-2 shadow-lg">
              <Plus className="h-5 w-5" />Create New Character
            </Button>
          </Link>
        </div>
          ...
          <div className="text-center py-20 animate-fade-in">
            ...
            <Link to="/">
              <Button variant="gold" size="lg" className="gap-2">
                <Plus className="h-5 w-5" />Generate Your First Character
              </Button>
            </Link>
          </div>
        )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
                    ...
                    <div className="md:col-span-2 p-6 flex flex-col justify-between">
                      <div>
                        ...
                        {featured.concept && (
                          <p className="text-sm text-muted-foreground italic line-clamp-2">"{featured.concept}"</p>
                        )}
                      </div>
                        ...
                        {featured.play_guide_content && (
                          <Button variant="outline" size="sm" className="gap-1.5"
                            ...
                            {actionLoading?.id === featured.id && actionLoading.type === 'guide'
                              ? <Loader2 className="h-4 w-4 animate-spin" /> : <BookOpen className="h-4 w-4" />}
                            Play Guide
                          </Button>
                        )}
                        <Button variant="outline" size="sm" className="gap-1.5 border-green-500/40 text-green-400 hover:bg-green-500/10"
                          onClick={() => handleLevelingGuide(featured)}
                          disabled={actionLoading?.id === featured.id && actionLoading.type === 'level'}>
                          {actionLoading?.id === featured.id && actionLoading.type === 'level'
                            ? <Loader2 className="h-4 w-4 animate-spin" /> : <TrendingUp className="h-4 w-4" />}
                          {featured.leveling_guide_content ? 'Level Guide ✓' : 'Generate Level Guide'}
                        </Button>
                        <Button variant="outline" size="sm" className="gap-1.5"
            ...
            {(rest.length > 0 || generating.length > 0) && (
              <section className="animate-fade-in" style={{ animationDelay: '150ms' }}>
                ...
                <div className="bg-gradient-card rounded-2xl border border-border/50 overflow-hidden divide-y divide-border/40">
                  {generating.map((c) => <GeneratingRow key={c.id} character={c} />)}
                  {rest.map((c) => (
                    <CharacterRow
                      key={c.id} character={c} actionLoading={actionLoading}
                      onSheet={() => handleSheet(c)}
                      onPlayGuide={() => handlePlayGuide(c)}
                      onLevelGuide={() => handleLevelingGuide(c)}
                      onPrint={handlePrint}
...
function CharacterRow({ character: c, actionLoading, onSheet, onPlayGuide, onLevelGuide, onPrint, onDelete }: RowProps) {
    ...
    <div className="flex items-center gap-4 px-4 py-3 hover:bg-muted/20 transition-colors group">
      <Link to={`/character/${c.id}`} className="flex-shrink-0">
        <div className="w-12 h-12 rounded-lg overflow-hidden border border-border/50">
          <PortraitWithSkeleton portraitUrl={c.portrait_url} characterName={c.character_name}
            characterId={c.id} className="w-full h-full object-cover" />
        </div>
      </Link>
      <Link to={`/character/${c.id}`} className="flex-1 min-w-0">
        <p className="font-semibold truncate group-hover:text-primary transition-colors">{c.character_name}</p>
        <p className="text-xs text-muted-foreground truncate">
      ...
      <div className="hidden sm:flex items-center gap-3 text-xs text-muted-foreground flex-shrink-0">
        <span className="flex items-center gap-1"><Heart className="h-3 w-3 text-destructive" />{stats?.hitPoints?.maximum ?? '—'}</span>
        <span className="flex items-center gap-1"><Shield className="h-3 w-3 text-primary" />{stats?.armorClass ?? '—'}</span>
        <span className="flex items-center gap-1"><Zap className="h-3 w-3 text-gold" />+{stats?.proficiencyBonus ?? 2}</span>
        <span className="px-1.5 py-0.5 bg-muted/50 rounded">{c.ruleset}</span>
      </div>
      <div className="flex items-center gap-1 flex-shrink-0">
        <Link to={`/character/${c.id}`}>
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="View Sheet"><Eye className="h-4 w-4" /></Button>
        </Link>
        {c.play_guide_content && (
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="Play Guide"
        ...
        <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title={c.leveling_guide_content ? 'Level Guide (cached)' : 'Generate Level Guide'}
          onClick={onLevelGuide} disabled={isLoading('level')}>
          {isLoading('level')
            ? <Loader2 className="h-4 w-4 animate-spin" />
            : <TrendingUp className={`h-4 w-4 ${c.leveling_guide_content ? 'text-green-400' : ''}`} />}
        </Button>
        <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="Download Sheet"
        ...
        <Button variant="ghost" size="sm" className="h-8 w-8 p-0" title="Print" onClick={onPrint}>
